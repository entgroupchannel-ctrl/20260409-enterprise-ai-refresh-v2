...
CREATE INDEX idx_invoice_share_links_invoice ON public.invoice_share_links(invoice_id);
CREATE INDEX idx_invoice_share_links_token ON public.invoice_share_links(token);
...
CREATE TABLE public.invoice_share_access_log (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  share_link_id uuid NOT NULL REFERENCES public.invoice_share_links(id) ON DELETE CASCADE,
...
CREATE INDEX idx_invoice_share_access_log_link ON public.invoice_share_access_log(share_link_id);
CREATE INDEX idx_invoice_share_access_log_time ON public.invoice_share_access_log(accessed_at DESC);

ALTER TABLE public.invoice_share_access_log ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins read invoice share access log"
  ON public.invoice_share_access_log FOR SELECT
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'sales'));

CREATE OR REPLACE FUNCTION public.get_shared_invoice(p_token text, p_action text DEFAULT 'view')
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  v_link public.invoice_share_links%ROWTYPE;
...
BEGIN
  ...
  SELECT * INTO v_link FROM public.invoice_share_links WHERE token = p_token;
  IF NOT FOUND THEN
  ...
  SELECT COALESCE(jsonb_agg(to_jsonb(i.*) ORDER BY i.display_order NULLS LAST, i.created_at), '[]'::jsonb)
    INTO v_items
    FROM public.invoice_items i WHERE i.invoice_id = v_invoice.id;
  ...
  INSERT INTO public.invoice_share_access_log(share_link_id, action)
...
GRANT EXECUTE ON FUNCTION public.get_shared_invoice(text, text) TO anon, authenticated;