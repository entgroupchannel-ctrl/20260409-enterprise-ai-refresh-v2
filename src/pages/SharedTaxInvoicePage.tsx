...
import TaxInvoicePDFTemplate from '@/components/admin/TaxInvoicePDFTemplate';
import { format } from 'date-fns';
import { th } from 'date-fns/locale';

export default function SharedTaxInvoicePage() {
  ...
  const load = async (action: 'view' | 'download') => {
    setLoading(true);
    try {
      const { data: result, error: rpcError } = await (supabase as any).rpc('get_shared_tax_invoice', {
        p_token: token,
        p_action: action,
      });
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      await (supabase as any).rpc('get_shared_tax_invoice', { p_token: token, p_action: 'download' });
      const html2pdf = (await import('html2pdf.js')).default;
      const element = document.getElementById('tax-invoice-pdf-template');
      if (!element) return;
      await html2pdf().set({
        margin: [15, 15, 20, 15],
        filename: `${data.taxInvoice.tax_invoice_number}.pdf`,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-muted/30">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">กำลังโหลดใบกำกับภาษี...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    const titleMap: Record<string, string> = {
      ...
      tax_invoice_not_found: 'ไม่พบใบกำกับภาษี',
    };
    return (
  ...
  if (!data) return null;
  const { taxInvoice, items, company, invoiceNumber, link } = data;
    ...
    <>
      ...
      <div className="min-h-screen bg-muted/30">
        <div className="bg-background border-b sticky top-0 z-10">
          <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-semibold">ใบกำกับภาษี {taxInvoice.tax_invoice_number}</div>
        ...
        <div className="max-w-5xl mx-auto p-4">
          <div id="tax-invoice-pdf-template" className="bg-white shadow-lg">
            {company ? (
