...
export default function SharedInvoicePage() {
  ...
  const load = async (action: 'view' | 'download') => {
    setLoading(true);
    try {
      const { data: result, error: rpcError } = await (supabase as any).rpc('get_shared_invoice', {
        p_token: token,
        p_action: action,
      });
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      await (supabase as any).rpc('get_shared_invoice', { p_token: token, p_action: 'download' });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-muted/30">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">กำลังโหลดใบแจ้งหนี้...</p>
        </div>
      </div>
    );
  }
  ...
  if (!data) return null;
  const { invoice, items, company, banks, link } = data;
        ...
        <div className="bg-background border-b sticky top-0 z-10">
          <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-semibold">ใบแจ้งหนี้ {invoice.invoice_number}</div>
