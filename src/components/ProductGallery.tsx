...
const ProductGallery = ({ images, alt, autoPlayInterval = 4000 }: ProductGalleryProps) => {
  ...
  if (images.length === 1) {
    return <img src={images[0]} alt={alt} className="max-h-[240px] object-contain mx-auto" loading="lazy"  decoding="async"/>;
  }
    ...
    >
      ...
      <div className="flex gap-2 mt-3 justify-center items-center">
        {images.map((img, i) => (
          ...
          >
            <img src={img} alt="" className="w-full h-full object-contain bg-background/50 p-0.5" loading="lazy"  decoding="async"/>
          </button>
        ))}
      </div>
