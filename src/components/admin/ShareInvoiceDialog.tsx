...
import {
  Share2, Copy, Eye, Download, Loader2, Link as LinkIcon, Ban, Plus, ExternalLink,
} from 'lucide-react';
...
export default function ShareInvoiceDialog({ open, onOpenChange, invoiceId, invoiceNumber }: Props) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const { error } = await (supabase as any).from('invoice_share_links').insert({
  ...
  const buildUrl = (token: string, action: 'view' | 'download' = 'view') => {
    ...
    return action === 'download' ? `${base}/inv/share/${token}?download=1` : `${base}/inv/share/${token}`;
  };
