...
export default function ShareTaxInvoiceDialog({ open, onOpenChange, taxInvoiceId, taxInvoiceNumber }: Props) {
  ...
  useEffect(() => {
    if (open && taxInvoiceId) loadLinks();
    if (!open) setDays(7);
  }, [open, taxInvoiceId]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const { error } = await (supabase as any).from('tax_invoice_share_links').insert({
  ...
  const buildUrl = (token: string, action: 'view' | 'download' = 'view') => {
    ...
    return action === 'download' ? `${base}/tx/share/${token}?download=1` : `${base}/tx/share/${token}`;
  };
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            แชร์ใบกำกับภาษี {taxInvoiceNumber}
          </DialogTitle>
          <DialogDescription>
